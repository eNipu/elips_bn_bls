...
void EFp_ECD_return_lambda(EFp *ANS,Fp *lambda,EFp *P){
...
void EFp_ECA_return_lambda(EFp *ANS,Fp *lambda,EFp *P1, EFp *P2){
    if(P1->infinity==1){
        EFp_set(ANS,P2);
        return;
...
void ff_ltt_vtt_for_tate(Fp12 *f,EFp *T,EFp12 *Q){
    EFp Next_T;
    EFp_init(&Next_T);
    Fp12 tmp1,tmp2,tmp3;
    Fp12_init(&tmp1);
    ...
    EFp_ECD_return_lambda(&Next_T,&lambda,T);
...
void f_ltp_vtp_for_tate(Fp12 *f,EFp *T,EFp *P,EFp12 *Q){
    EFp Next_T;
    EFp_init(&Next_T);
    Fp12 tmp1,tmp2,tmp3;
    Fp12_init(&tmp1);
    ...
    EFp_ECA_return_lambda(&Next_T,&lambda,T,P);
