...
static void test_scalar_edges(void)
{
     ...
     * is where a decomposition is most likely to be off by one. Both groups and
     * both families: BN G1 rounds against a lattice basis rather than dividing
     * in a base, so its off-by-one looks different from the others. */
    ep_t G1p, O1;
    ep_generator(&G1p);
    ep_set_infinity(&O1);

    int agree2 = 1, agree1 = 1, atinf = 1;
    for (int i = 0; i < 3; i++) {
        memset(k, 0, sizeof k);
        if (i == 1) k[0] = 1;
        ...
        ep2_t plain2, glv2;
        ep2_mul(&plain2, &H, k, ELIPS_ORDER_BITS);
        ep2_mul_glv(&glv2, &H, k, ELIPS_ORDER_BITS);
        if (!ep2_same(&plain2, &glv2)) agree2 = 0;

        ep_t plain1, glv1;
        ep_mul(&plain1, &G1p, k, ELIPS_ORDER_BITS);
        ep_mul_glv(&glv1, &G1p, k, ELIPS_ORDER_BITS);
        fp_t ax, ay, bx, by;
        int ai = ep_to_affine(ax, ay, &plain1), bi = ep_to_affine(bx, by, &glv1);
        if (ai != bi || (ai && !(fp_eq(ax, bx) && fp_eq(ay, by)))) agree1 = 0;

        /* The identity is a legitimate input. phi(O) is O and the whole table
         * collapses to it, so the complete addition law has to carry it
         * through without a special case. */
        ep_mul_glv(&glv1, &O1, k, ELIPS_ORDER_BITS);
        if (!ep_is_infinity(&glv1)) atinf = 0;
    }
    ok(agree2, "ep2_mul_glv agrees with the plain ladder at k = 0, 1 and r-1");
    ok(agree1, "ep_mul_glv agrees with the plain ladder at k = 0, 1 and r-1");
    ok(atinf, "ep_mul_glv on the identity stays the identity");
}
