...
int main(void)
{
    ...
    /* Same reason as pairing_test.c: LeakSanitizer is on by default under Asan
     * on Linux and does not care that the process was about to exit. */
    EFp2_clear(&L1); EFp2_clear(&L2); EFp2_clear(&L3);
    EFp12_clear(&G2);
